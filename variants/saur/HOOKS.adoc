...
This document describes the git hooks available for saur-ssg
development.
...
*Checks:* - Code formatting (`+deno fmt --check+`) - Linting
(`+deno lint+`) - No hardcoded secrets - SPDX headers present - No
...
*Checks:* - Tests pass (`+deno test+`) - All adapters have valid MCP
interface - Required SCM files present (META.scm, STATE.scm,
ECOSYSTEM.scm)
...
Documentation - `+style+` - Formatting (no code change) - `+refactor+` -
Code restructuring - `+test+` - Adding/fixing tests - `+chore+` -
Maintenance - `+perf+` - Performance - `+ci+` - CI/CD changes -
`+build+` - Build system - `+security+` - Security fixes
...
These hooks run the same checks as CI, allowing you to catch issues
