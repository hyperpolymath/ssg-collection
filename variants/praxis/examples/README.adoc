...
Each SSG has its own configuration format. The examples in `+config/+`
show how to configure common options.

==== Templates

Templates in `+templates/+` are generic examples that work with most
SSGs. Adjust the syntax based on your chosen SSG’s template engine:
...
. Adjust template syntax for your SSG
