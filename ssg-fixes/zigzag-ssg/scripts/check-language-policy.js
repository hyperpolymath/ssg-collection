...
//
// zigzag-ssg Language Policy Enforcement
// This SSG MUST be written in Zig - no exceptions

const fs = require("fs");
const path = require("path");

const CORE_LANGUAGE = "Zig";
const CORE_EXTENSIONS = [".zig"];
...
function findFiles(dir, extensions, results = []) {
  ...
  for (const file of files) {
    ...
    if (stat.isDirectory() && !file.startsWith(".") && file !== "node_modules" && file !== "zig-cache" && file !== "zig-out") {
...
function main() {
  ...
  console.log(`zigzag-ssg Language Policy Check`);
