...
SQLite database operations demonstrating Ephapax’s linear type system
...
[arabic]
. *No use-after-close* - Connection consumed on close
. *No forgotten transactions* - Must explicitly commit/rollback
. *No resource leaks* - Region-based deallocation
. *Clear ownership* - Query results owned by caller
