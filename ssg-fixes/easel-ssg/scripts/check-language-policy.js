...
//
// easel-ssg Language Policy Enforcement
// This SSG MUST be written in Piet - no exceptions
// Piet programs are image files (PNG, GIF, PPM)

const fs = require("fs");
const path = require("path");

const CORE_LANGUAGE = "Piet";
const CORE_EXTENSIONS = [".png", ".gif", ".ppm", ".piet.txt"];
const CORE_DIRECTORIES = ["engine/", "patterns/"];
...
function checkCoreLanguage() {
  ...
  console.log("   Note: Piet programs are encoded as images (PNG, GIF, PPM)");
  ...
  if (!hasCore) {
    ...
    console.error(`   Piet programs are image files: ${CORE_EXTENSIONS.join(", ")}`);
...
function main() {
  ...
  console.log(`easel-ssg Language Policy Check`);
