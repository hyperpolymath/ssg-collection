# MCP Adapter for parallax-ssg

This directory contains the **ReScript MCP adapter** that connects parallax-ssg to the poly-ssg-mcp hub.
...
adapters/
├── src/
│   └── ParallaxAdapter.res  # ReScript adapter source
...
**parallax-ssg uses Chapel** as its primary implementation language.
The adapter wraps the Chapel runtime to provide MCP protocol access.

## Connection to Hub

This adapter exports:
- `name` - Satellite identifier
- `language` - Primary implementation language (Chapel)
...
ALL SSG logic must remain in Chapel.
The adapter may ONLY:
- Execute commands in the Chapel runtime
