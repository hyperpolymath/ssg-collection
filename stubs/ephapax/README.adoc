= Ephapax SSG - Static Site Generator Written in Idris2
:toc:
:toclevels: 3

== Overview

Ephapax SSG is a general-purpose static site generator written in *Idris2* with formally verified correctness properties.

Part of the **poly-ssg** family (alongside wokelang-ssg, casket-ssg, hackenbush-ssg), it provides:

1. **SSG Tool** - Generate static sites from markdown + frontmatter
2. **Formal Verification** - Dependent types prove correctness at compile-time
3. **Zero Runtime Errors** - Totality checking ensures all code paths terminate
