# MCP Adapter for rescribe-ssg

This directory contains the **ReScript MCP adapter** that connects rescribe-ssg to the poly-ssg-mcp hub.
...
adapters/
├── src/
│   └── RescribeAdapter.res  # ReScript adapter source
...
**rescribe-ssg uses ReScript** as its primary implementation language.
The adapter wraps the ReScript runtime to provide MCP protocol access.

## Connection to Hub

This adapter exports:
- `name` - Satellite identifier
- `language` - Primary implementation language (ReScript)
...
ALL SSG logic must remain in ReScript.
The adapter may ONLY:
- Execute commands in the ReScript runtime
